...
public class DarkDockingListener implements DockingListener {
	...
	public void dockingWillChange(DockingEvent e) {
		if ((e.getDestinationDock() != workspace.getWorkspaceDock()) && (workspace.getWorkspaceDock().getChildDockCount() == 1)) {
			workspace.setFillerDockable(
					GUIManager.externalWithListener(new DefaultDockable("Workspace", new WorkspaceFiller(), "Workspace"), this));
			workspace.setFillerDock(
					workspace.getDockFactory().createDock(workspace.getFillerDockable(), DockingMode.SINGLE));
